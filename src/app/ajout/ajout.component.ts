...
export class AjoutComponent implements OnInit , AfterViewInit {
  ...
  constructor (private fb : FormBuilder,private router :Router,private routeActive : ActivatedRoute){}
...
// les methodes
  ...
  public matriculeIsExist(num :string){
    const tab = this.data.map(tab => tab.matricule)
  ...
  public addData(){
    if(typeof localStorage !== 'undefined'){
      if(this.form.valid){
        ...
        const exist =this.matriculeIsExist(addEtu.matricule)
        if(exist===-1){
          this.data.push(addEtu)
          const newData = JSON.stringify(this.data)
          localStorage.setItem(this.list,newData)
    ...
    if(matricule){
      if(typeof localStorage !== 'undefined'){
        if(this.form.valid){
          ...
          const indexEtuUpdate = this.data.findIndex((etu)=>etu.matricule===matricule)
          this.data.splice(indexEtuUpdate,1,updEtu)

          localStorage.setItem(this.list,JSON.stringify(this.data))
